const getDistLocation = (importName, opts) => {
    return `native-components-web/dist/index`;
};

const isNativeComponentRequire = (t, node) => {
    ...
    return (
        ...
        (init.arguments[0].value === 'native-components' || init.arguments[0].value === 'native-components-web')
    );
};

const isNativeComponentModule = ({source, specifiers}) =>
    source &&
    (source.value === 'native-components' || source.value === 'native-components-web') &&
    specifiers.length;

module.exports = function ({types: t}) {
    return {
        name: 'Rewrite native-components to native-components-web',
        visitor: {
            ImportDeclaration(path, state) {
                const {specifiers} = path.node;
                if (isNativeComponentModule(path.node)) {
            ...
            VariableDeclaration(path, state) {
                if (isNativeComponentRequire(t, path.node)) {
